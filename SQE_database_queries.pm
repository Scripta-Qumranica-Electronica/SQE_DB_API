...
use constant {
    ...
    GET_LOGIN => 'SELECT user_id FROM user WHERE user_name = ? AND pw = SHA2(?, 224);'
};
...
use constant
    LINE_OWNERS => ['line_data','line_to_sign'];

use constant
    COL_OWNERS => ['col_data','col_to_line'];

use constant
    SCROLL_OWNERS => ['scroll_data','scroll_to_col'];
...
HEREDOC
    ...
    GET_SIGN_CHAR_OWNER_IDS => << 'HEREDOC',
    select
...
HEREDOC
    ...
    GET_SIGN_CHAR_READING_DATA_OWNER_IDS => << 'HEREDOC',
    INSERT
    select
