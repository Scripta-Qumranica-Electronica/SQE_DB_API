...
sub process_cgi {
    ...
    #We expect the cgi-Object
    my $data = { cgi => shift };

    # We don't want get-parameter
    return get_error_string(SQE_Error::NO_GET_REQUESTS)
      if $data->{cgi}->url_param;

    # Check user credits and get databasehandler if ok.
    ( $data->{dbh}, my @error ) =
      SQE_database::get_login_sqe( $data->{cgi}->param('USER_NAME'),
        $data->{cgi}->param('PASSWORD') );
    return get_error_string(@error) if not $data->{dbh};
...
sub get_scroll_names {
    ...
    my $scroll_data_ref =
      $data->{dbh}
      ->selectcol_arrayref( SQE_API::Queries::GET_SCROLLS, { Columns => [2] },
        $scroll_name, 0, 0 );
    $data->{dbh}->disconnect;
...
sub get_fragment_names {
    ...
    my $fragment_data_ref =
      $data->{dbh}->selectcol_arrayref( SQE_API::Queries::GET_FRAGMENTS,
        { Columns => [2] },
        $scroll_id, $fragment_name, 0, 0
      );
    $data->{dbh}->disconnect;
...
sub get_line_names {
    ...
    my $fragment_ids =
      $data->{dbh}->selectall_arrayref( SQE_API::Queries::GET_FRAGMENTS,
        undef, $scroll_id, $fragment_name, 0, 0 );
    if ( @$fragment_ids == 0 ) {
        $data->{dbh}->disconnect;
    ...
    my $line_data_ref =
      $data->{dbh}->selectcol_arrayref( SQE_API::Queries::GET_LINES,
        { Columns => [1] },
        $fragment_ids->[0]->[0],
        $line_name, 0, 0
      );
...
sub get_unique_scroll {
    my $data        = shift;
    my $scroll_name = quote( $data->{cgi}->param('SCROLL') );
    my $scroll_id;
    my $scroll_name_sth = $data->{dbh}->prepare(SQE_API::Queries::GET_SCROLLS);
    $scroll_name_sth->execute( $scroll_name, 0, 0 );
...
sub get_text {
    ...
    my $out_text;
    my $postion_sth =
      $data->{dbh}->prepare(SQE_API::Queries::GET_SIGN_POSITIONS);
    if ( $format eq 'JSON' ) {
        $data->{format_template} =
          SQE_API::format_json->new( \$out_text, $postion_sth );
    }
    elsif ( $format eq 'QWB_HTML' ) {
        $data->{format_template} =
          SQE_API::format_qwb_html->new( \$out_text, $postion_sth );
    }
    else {
    ...
    if ( $data->{line_name} ) {
        my $get_line_break_sth =
          $data->{dbh}->prepare(SQE_API::Queries::GET_LINE_BREAK);
        $get_line_break_sth->execute( $fragment_ids->[0]->[0],
            $data->{line_name}, 'LINE_START', 0, 0 );
        if ( not $data->{fragment_start} =
            ( $get_line_break_sth->fetchrow_array )[0] )
        {
        ...
        $get_line_break_sth->execute( $fragment_ids->[0]->[0],
            $data->{line_name}, 'LINE_END', 0, 0 );
        if ( not $data->{fragment_end} =
            ( $get_line_break_sth->fetchrow_array )[0] )
        {
            $get_line_break_sth->finish;
            $postion_sth->finish;
        ...
        $data->{get_line_data_sth} =
          $data->{dbh}->prepare(SQE_API::Queries::GET_LINE_DATA);
        $data->{get_sign_sth} =
          $data->{dbh}->prepare(SQE_API::Queries::GET_SIGN);
        $data->{get_nex_position_sth} =
          $data->{dbh}->prepare(SQE_API::Queries::GET_NEXT_POSITION);
        $data->{get_sign_data_sth} =
          $data->{dbh}->prepare(SQE_API::Queries::GET_SIGNS_OF_LINE);

        get_fragment( $fragment_ids->[0], $data );
        $data->{get_line_data_sth}->finish;
        $data->{get_sign_sth}->finish;
        $data->{get_nex_position_sth}->finish;
        $postion_sth->finish;
    }
    else {
        my $get_fragment_break_sth =
          $data->{dbh}->prepare(SQE_API::Queries::GET_FRAGMENT_BREAK);
        $data->{get_line_data_sth} =
          $data->{dbh}->prepare(SQE_API::Queries::GET_LINE_DATA);
        $data->{get_sign_sth} =
          $data->{dbh}->prepare(SQE_API::Queries::GET_SIGN);
        ...
        foreach my $single_data (@$fragment_ids) {
            ...
            $get_fragment_break_sth->execute( $single_data->[0], 'COLUMN_END',
                0, 0 );
            if ( not $data->{fragment_end} =
                ( $get_fragment_break_sth->fetchrow_array )[0] )
            {
            ...
            $data->{get_sign_data_sth} =
              $data->{dbh}->prepare(SQE_API::Queries::GET_SIGNS_OF_COLUMN);
...
sub get_fragment {
    ...
    my $sign_type;
       ...
       my $dbhx         = SQE_DBH->new( $data->{dbh} );
      my $sign_ids_ref = $dbhx->selectall_hashref_with_key_from_column
       ( SQE_API::Queries::GET_ALL_SIGNS_IN_FRAGMENT, 2, $fragment_data->[0]);

    do {
        $data->{get_line_data_sth}->execute( $next_sign_id, 0, 0 );
        ( $next_sign_id, $data->{line_name} ) =
          $data->{get_line_data_sth}->fetchrow_array;

        $data->{format_template}->format_line( $data->{line_name} );
        do {
            my $array_ref = $sign_ids_ref->{$next_sign_id};
            $sign_data_array_ref = shift @{$array_ref};
            $sign_type           = $sign_data_array_ref->[3];

            $next_sign_id = $sign_data_array_ref->[0];

            #           $next_sign_id        = $sign_data_array_ref->[0];
            while ($sign_data_array_ref) {
                $data->{format_template}->format_sign($sign_data_array_ref)
                  if $sign_data_array_ref->[3] != 9;
                $sign_data_array_ref = shift @{$array_ref};
            }
        } until ( $sign_type == 9 || $next_sign_id == $data->{fragment_end} );
    } until ( $next_sign_id == $data->{fragment_end} );
...
#        $data->{format_template}->format_line( $data->{line_name} );
#        do {
#            $data->{get_sign_sth}->execute($next_sign_id, 0, 0);
#            $sign_data_array_ref = $data->{get_sign_sth}->fetchrow_arrayref;
#            $sign_type           = $sign_data_array_ref->[3];
#            $data->{get_nex_position_sth}->execute($next_sign_id, 0, 0);
#            $next_sign_id        = $data->{get_nex_position_sth}->fetchrow_arrayref->[0];
# #           $next_sign_id        = $sign_data_array_ref->[0];
#            while ($sign_data_array_ref) {
#                $data->{format_template}->format_sign($sign_data_array_ref)
#                  if $sign_data_array_ref->[3] != 9;
