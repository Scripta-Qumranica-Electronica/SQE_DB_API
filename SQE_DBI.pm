...
=cut
    ...
    sub _set_scroll_version {
        ...
        $self->
        $self->{private_SQE_DBI_data}->{SESSION}->{SCROLL_VERSION_ID} =
          $scroll_version_id;
...
"INSERT IGNORE INTO ${table}_owner (${table}_id, scroll_version_id) VALUES (?,?)";
    ...
    sub clone_scroll_version {
        my ( $self, $old_scroll_version_id, $new_scroll_version_id ) = @_;
        my $table_names_sth =
          $self->prepare_cached(SQE_DBI_queries::GET_OWNER_TABLE_NAMES);
        $table_names_sth->execute;
        my $name_ref;
        while ( $name_ref = $table_names_sth->fetchrow_arrayref ) {
            my $query       = SQE_DBI_queries::CLONE_SCROLL_VERSION;
            my $owner_table = $name_ref->[0];
    ...
    sub create_new_scrollversion {
        ...
        $self->set_scroll_version_group_admin( $scroll_version_group_id,
            $self->user_id );

        # Create a new scroll_version as member of the new group
        $sth = $self->prepare_cached(SQE_DBI_queries::NEW_SCROLL_VERSION);
    ...
    sub delete_scroll_version {
        ...
        while ( $name_ref = $table_names_sth->fetchrow_arrayref ) {
            ...
            my $owner_table = $name_ref->[0];
            $query =~ s/\*OWNER\*/$owner_table/go;
            $query =~ s/\*SVID\*/$scroll_version_id/go;
            $self->do($query);
        }
        $self->do( SQE_DBI_queries::DELETE_SCROLLVERSION_FROM_ACTIONS,
            undef, $scroll_version_id );
        $self->do(SQE_DBI_queries::DELETE_EMPTY_SCROLLVERSION_GROUPS);
    ...
    sub remove_owner {
        ...
        my $id    = shift;
        my $query =
"DELETE FROM ${table}_owner WHERE ${table}_id = ?  AND scroll_version_id = ?";
        my $sth = $self->_get_prepared_sqe_sth($table, 'delete', $query);
        my $result = $sth->logged_execute($id, $self->scroll_version_id);
        $sth->finish;
        return $result;
    }
...
Tests whether a sign char already owns the given attribute. If true and the attribute
ewven belongs to the current scrollversion than undef is returne, if it belongs to
...
=cut
    ...
    sub set_sign_char_attribute {
        my ($self, $sign_char_id, $attribute_value_id, $numeric_value, $sequence) = @_;
        $sequence = $sequence ? $sequence: 0;
        my $sth;
        if ($numeric_value) {
            $sth = $self->prepare_cached(SQE_DBI_queries::GET_SIGN_CHAR_ATTRIBUTE_NUMERIC);
            $sth->execute($sign_char_id, $attribute_value_id, $sequence, $numeric_value);
        }
        else {
            $sth = $self->prepare_cached(SQE_DBI_queries::GET_SIGN_CHAR_ATTRIBUTE);
            $sth->execute($sign_char_id, $attribute_value_id, $sequence);
        }
        my ($sign_char_attribute_id, $scroll_version_id);
        $sth->bind_columns(\$sign_char_attribute_id, \$scroll_version_id);
        my $new_sign_char_attribute_id=0;
        SVTEST: while ($sth->fetch) {
            if (defined $scroll_version_id && $scroll_version_id == $self->scroll_version_id) {
                $new_sign_char_attribute_id = undef;
                last SVTEST;
            }
            $new_sign_char_attribute_id=$sign_char_attribute_id;
        }
        $sth->finish;

        return $sign_char_attribute_id if !defined $new_sign_char_attribute_id;

        if ($new_sign_char_attribute_id==0) {
            $sth = $self->prepare_cached(SQE_DBI_queries::NEW_SIGN_CHAR_ATTRIBUTE);
            $sth->execute($sign_char_id, $attribute_value_id, $sequence);
    ...
    sub delete_sign_char_attributes_for_attribute {
        ...
        my $sth = $self->prepare_cached(SQE_DBI_queries::GET_ALL_SIGN_CHAR_ATTRIBUTES_FOR_ATTRIBUTE);
        $sth->execute($sign_char_id, $self->scroll_version_id, $attribute_value_id);
        my $sign_char_attribute_id;
        $sth->bind_col(1, \$sign_char_attribute_id);
        while ($sth->fetch) {
            $self->remove_owner('sign_char_attribute', $sign_char_attribute_id);
        }
    }
...
#   id of the record to be duplicated
    sub remove_entry {
        ...
        return ( undef, SQE_Error::QWB_RECORD ) if $self->scrollversion == 1;
        $self->start_logged_action;
        my $result = $self->remove_owner( $table, $id );
        $self->stop_logged_action;
        if ( $result > 0 ) {
            return $result;
        }
...
#   new scrollversion
    sub set_scrollversion {
        ...
        $scroll_version_id = $self->session->set_scrollversion($scroll_version_id);
...
#   the affected table
    sub set_action {
        ...
        $self->{private_sth}->{STH} = $self->{Database}->prepare_cached(SQE_DBI_queries::NEW_SINGLE_ACTION);
    }
    ...
    sub logged_execute {
        ...
        $self->{private_sth}->{STH}->execute( $dbh->action_log_id,
